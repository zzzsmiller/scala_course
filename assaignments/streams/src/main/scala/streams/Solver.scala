...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    if (neighbors.isEmpty) Stream.Empty
      ...
      if (explored contains neighbor._1) newNeighborsOnly(neighbors.tail, explored)
      else {
        neighbor #:: newNeighborsOnly(neighbors.tail, explored + neighbor._1)
      }
    }
  }
   ...
   * position.
   */
  lazy val solution: List[Move] = pathsToGoal.apply(0)._2 
}
